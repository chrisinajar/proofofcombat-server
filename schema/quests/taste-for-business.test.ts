...
describe("Taste for Business Quest", () => {
  ...
  beforeEach(() => {
    ...
    mockContext = { io: { sendNotification: jest.fn() } };
  });
  ...
  it("should trigger first event at exactly 1000", () => {
    ...
    expect(hero.questLog.tasteForBusiness?.lastEvent?.id).toBe(
      "TasteForBusiness-test-hero-1-aFineCustomer",
    );
    expect(hero.questLog.tasteForBusiness?.lastEvent?.message).toBe(
  ...
  it("should trigger second event at 2000", () => {
    ...
    expect(hero.questLog.tasteForBusiness?.lastEvent?.id).toBe(
      "TasteForBusiness-test-hero-1-aLittleOpportunity",
    );
    expect(hero.questLog.tasteForBusiness?.lastEvent?.message).toBe(
