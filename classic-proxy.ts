...
function pickTargetUrl(targets: ClassicTargets, isSecure: boolean): URL {
  if (typeof targets === "string") {
    return new URL(targets);
  }
  const chosen = isSecure
    ? targets.https || targets.http
    : targets.http || targets.https;
  if (!chosen) {
    throw new Error("No classic proxy target configured");
  }
...
export function createClassicProxy(targets: ClassicTargets): RequestHandler {
  return (req, res) => {
    ...
    const path =
      `${targetUrl.pathname?.replace(/\/$/, "") || ""}${req.url}` || "/";
    ...
    const proxyReq = (isHttps ? https : http).request(options, (proxyRes) => {
      ...
      const responseHeaders = { ...proxyRes.headers } as Record<
        string,
      ...
      if (location) {
        ...
        const rewrite = (v: string) =>
          v.startsWith(targetOrigin) ? v.replace(targetOrigin, "/classic") : v;
      ...
      res.writeHead(
        ...
        responseHeaders as http.OutgoingHttpHeaders,
      );
...
type UpgradeTargets =
  | string
  | { http?: string; https?: string; socket?: string };
...
// Handle HTTP long-polling endpoints for Socket.IO when the client requests 
// '/socket.io/*' at the server root (e.g., classic UI loaded under /classic but 
// its Socket.IO client uses default root path). This proxies directly to the
// classic socket target without stripping a '/classic' prefix.
export function createClassicSocketHttpProxy(targets: UpgradeTargets): RequestHandler {
  return (req, res) => {
